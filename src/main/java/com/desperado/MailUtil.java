...
public class MailUtil {
    ...
    // 收件人邮箱（替换为自己知道的有效邮箱）
    public static List<String> receiveMailAccount = Arrays.asList("xxx@icloud.com", "xxx@qq.com");
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, "gxy", "UTF-8"));

        // 3. To: 收件人（可以增加多个收件人、抄送、密送）
        message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveMail, "XX用户", "UTF-8"));

        // 4. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("quickly", "UTF-8");

        // 5. Content: 邮件正文（可以使用html标签）（内容有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改发送内容）
        message.setContent("quickly!!!!，抢房", "text/html;charset=UTF-8");
